...
public class VideoListAdapter extends BaseAdapter implements YouTubeThumbnailView.OnInitializedListener {
    ...
    @Override
    public int getCount() {
        return YouTubeContent.YouTubeITEMS.size();
    }

    @Override
    public Object getItem(int position) {
        return YouTubeContent.YouTubeITEMS.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final YouTubeContent.YouTubeVideo item = YouTubeContent.YouTubeITEMS.get(position);

        if (convertView == null) {
            //Create the row
            final LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            convertView = inflater.inflate(R.layout.rowlayout_youtube, parent, false);
